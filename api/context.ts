...
export type LocalUser = { id: string; name: string; username: string };
export type TrpcContext = { req: Request; resHeaders: Headers; user?: LocalUser };
  ...
  try {
    ...
    if (token) {
      ...
      if (decoded?.username === "dentist") {
        ctx.user = { id: "dentist", name: "Dr. Dentist", username: "dentist" };
      }
    }
  } catch (error) {
